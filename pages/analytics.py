...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown(
        f"<div class='sb-about'><div class='sb-about-text'>"
...
with tab1:
    ...
    with c1:
        ...
        fig = go.Figure(go.Bar(
            x=ranges, y=counts,
            marker=dict(color=counts, colorscale=[[0,C_PURPLE],[0.5,C_GOLD],[1,C_TEAL]], showscale=False, line=dict(width=0)),
            hovertemplate="<b>%{x}</b><br>Movies: %{y:,}<extra></extra>",
        ))
        fig.update_layout(pl(title=f"Rating Distribution — All {TOTAL_MOVIES:,} Movies", xaxis_title="Rating Range", yaxis_title="Count", bargap=0.15))
    ...
    if f_pop_by_r:
        ...
        fig3 = go.Figure(go.Bar(
            x=[r["rating_group"] for r in f_pop_by_r], y=rg_vals,
            marker=dict(color=rg_vals, colorscale=[[0,C_PURPLE],[0.5,C_GOLD],[1,C_TEAL]], showscale=False, line=dict(width=0)),
            text=[f"{v:.1f}" for v in rg_vals], textposition="outside", textfont=dict(color=C_MUTED, size=11),
        ))
...
with tab2:
    ...
    with c2:
        ...
        fig2 = go.Figure(go.Bar(
            y=[p[1] for p in paired], x=[p[0] for p in paired], orientation="h",
            marker=dict(color=[p[0] for p in paired], colorscale=[[0,C_CORAL],[0.5,C_GOLD],[1,C_TEAL]], showscale=False, line=dict(width=0)),
            text=[f"{p[0]:.2f}" for p in paired], textposition="outside", textfont=dict(color=C_MUTED, size=11),
        ))
        fig2.update_layout(pl(title="Avg Rating by Genre", height=400, xaxis=dict(range=[5.0,6.8]), yaxis=dict(autorange="reversed")))
        st.plotly_chart(fig2, use_container_width=True)
...
with tab3:
    ...
    rd_pcts   = [round(c / f_total * 100, 1) if f_total > 0 else 0 for c in rd_counts]
    ...
    fig_rd.add_vline(x=mean_bucket_idx, line_dash="dot", line_color=C_GOLD, line_width=1.5,
                     annotation_text=f"Mean ≈ {f_avg_r}", annotation_font_color=C_GOLD, annotation_position="top right")
    fig_rd.update_layout(pl(title="Rating Distribution", xaxis_title="Rating Range", yaxis_title="Number of Movies", bargap=0.18, height=420))
    ...
    tiers = [
        ("⭐ Masterpiece  (9–10)", bucket_map.get("9-10",0),  C_TEAL),
        ("🥇 Excellent    (8–9)",  bucket_map.get("8-9",0),   C_GOLD),
        ("👍 Good         (7–8)",  bucket_map.get("7-8",0),   C_BLUE),
        ("😐 Average      (5–7)",  bucket_map.get("5-6",0)+bucket_map.get("6-7",0), C_PURPLE),
        ("👎 Below Avg   (0–5)",   bucket_map.get("0-2",0)+bucket_map.get("2-4",0)+bucket_map.get("4-5",0), C_CORAL),
    ]
    tier_counts = [t[1] for t in tiers]
    tier_pcts   = [round(c/f_total*100,1) if f_total>0 else 0 for c in tier_counts]
    fig_tier = go.Figure(go.Bar(
        y=[t[0] for t in tiers], x=tier_counts, orientation="h",
        marker=dict(color=[t[2] for t in tiers], opacity=0.88, line=dict(width=0)),
...
with tab4:
    ...
    with rc1:
        rec_genre = st.selectbox("Pick a Genre", [g["genre"] for g in overview["genre_stats"]], key="rec_genre")
    with rc2:
    ...
    if rec_err or not rec_data:
        ...
        for col, title_lbl, color, bucket_key in [
            ...
            with col:
                st.markdown(f"<div style='color:{color};font-weight:600;font-size:.9rem;margin-bottom:.6rem'>{title_lbl}</div>", unsafe_allow_html=True)
                for item in rec_data[bucket_key]:
                    genres_short = " · ".join(str(item["genres"]).split()[:2])
                    poster_url   = item.get("poster_url")
                    if poster_url:
                        poster_block = f"<img src='{poster_url}' style='width:52px;height:76px;object-fit:cover;border-radius:6px;flex-shrink:0;border:1px solid {C_BORDER}' />"
                    else:
                        poster_block = f"<div style='width:52px;height:76px;flex-shrink:0;border-radius:6px;border:1px dashed {C_BORDER};background:{C_SURFACE};display:flex;align-items:center;justify-content:center;font-size:.6rem;color:{C_MUTED};padding:4px;'>No Poster</div>"
                    st.markdown(
...
with tab5:
    ...
    if titles_err or not titles_data:
        ...
        with cc1:
            movie1 = st.selectbox("🎬 Movie 1", all_titles,
                index=all_titles.index("Inception") if "Inception" in all_titles else 0, key="m1")
        with cc2:
            movie2 = st.selectbox("🎬 Movie 2", all_titles,
                index=all_titles.index("The Dark Knight") if "The Dark Knight" in all_titles else 1, key="m2")
        ...
        if cmp_err or not compare_data:
            ...
            def poster_html(url):
                if url:
                    return f"<img src='{url}' style='width:120px;height:178px;object-fit:cover;border-radius:10px;margin-bottom:.8rem;border:2px solid {C_BORDER}' />"
            ...
            with col1:
                st.markdown(
                    f"<div class='compare-card {w1}'>{badge1}{poster_html(m1.get('poster_url'))}"
                    f"<div class='compare-title'>{movie1}</div>"
                    f"<div class='compare-big'>{rating1}</div><div class='compare-label'>Rating</div><br>"
                    f"<div class='compare-big' style='font-size:1.5rem;color:{C_TEAL}'>{round(pop1,1)}</div>"
                    f"<div class='compare-label'>Popularity</div><br>"
                    f"<div style='font-size:.85rem;color:{C_MUTED}'>{' · '.join(genres1[:4])}</div></div>",
                    unsafe_allow_html=True)
            with mid:
                st.markdown(f"<div style='display:flex;align-items:center;justify-content:center;height:100%;font-size:1.5rem;color:{C_MUTED};padding-top:3rem'>VS</div>", unsafe_allow_html=True)
            with col2:
                st.markdown(
                    f"<div class='compare-card {w2}'>{badge2}{poster_html(m2.get('poster_url'))}"
                    f"<div class='compare-title'>{movie2}</div>"
                    f"<div class='compare-big'>{rating2}</div><div class='compare-label'>Rating</div><br>"
                    f"<div class='compare-big' style='font-size:1.5rem;color:{C_TEAL}'>{round(pop2,1)}</div>"
                    f"<div class='compare-label'>Popularity</div><br>"
                    f"<div style='font-size:.85rem;color:{C_MUTED}'>{' · '.join(genres2[:4])}</div></div>",
                    unsafe_allow_html=True)

            st.markdown("<br>", unsafe_allow_html=True)
            st.markdown("<div class='section-hdr'>Detailed Comparison <span>METRICS</span></div>", unsafe_allow_html=True)
            def diff_color(a, b): return C_GREEN if a >= b else C_CORAL
            st.markdown(f"""
            ...
            <table class='movie-table'>
                ...
                <tr><td>⭐ Rating</td>
                    ...
                    <td><span class='rating-pill'>{rating_winner}</span></td></tr>
                <tr><td>🔥 Popularity</td>
                    ...
                    <td><span class='rating-pill'>{pop_winner}</span></td></tr>
                <tr><td>🎭 Genres</td><td>{" · ".join(genres1[:3])}</td><td>{" · ".join(genres2[:3])}</td><td>—</td><td>—</td></tr>
                <tr><td>🔗 Common Genres</td><td colspan='3'>{", ".join(common) if common else "None"}</td>
                    <td>{"✅ Match" if common else "❌ None"}</td></tr>
                <tr><td>📖 Overview Length</td><td>{ov_len1} words</td><td>{ov_len2} words</td>
